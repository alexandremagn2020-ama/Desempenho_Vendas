...
data_q1 = {
    ...
    'Real_Peso': [14180.00, 67825.00, 73275.00, 56720.00, 73149.00, 87924.00, 45028.00, 50418.00, 115611.50, 102832.00, 2825.00, 4203.00, 47402.00, 47751.00, 63168.00, 38206.00, 19999.00, 9969.00, 5370.00, 2750.00, 0.0],
    'Meta_PM': [18.76, 16.73, 17.05, 17.48, 17.45, 18.23, 15.50, 18.88, 20.05, 24.15, 24.00, 19.75, 17.75, 16.70, 18.60, 17.75, 20.08, 18.40, 20.15, 1.0, 0.0],
    'Real_PM': [17.97, 16.10, 16.82, 17.11, 17.11, 18.09, 15.27, 18.50, 19.73, 24.04, 23.43, 18.99, 17.53, 16.89, 15.65, 17.46, 19.77, 18.28, 21.22, 12.42, 0.0],
    'Meta_Pos': [20.0, 563.0, 578.0, 494.0, 592.0, 525.0, 444.0, 286.0, 39.0, 439.0, 17.0, 105.0, 552.0, 370.0, 366.0, 63.0, 180.0, 42.0, 47.0, 1.0, 0.0],
    'Real_Pos': [17.0, 570.0, 579.0, 487.0, 591.0, 512.0, 451.0, 278.0, 72.0, 458.0, 16.0, 82.0, 589.0, 354.0, 335.0, 61.0, 143.0, 23.0, 50.0, 15.0, 0.0],
    'Meta_Cad': [0.0, 12.0, 11.0, 20.0, 13.0, 17.0, 33.0, 40.0, 0.0, 6.0, 1.0, 40.0, 14.0, 30.0, 30.0, 3.0, 25.0, 25.0, 10.0, 1.0, 0.0],
    'Real_Cad': [0.0, 2.0, 15.0, 11.0, 21.0, 9.0, 18.0, 10.0, 10.0, 10.0, 0.0, 19.0, 17.0, 11.0, 10.0, 1.0, 38.0, 13.0, 18.0, 10.0, 0.0]
}
...
# Consolidação estrutural em dicionário nativo antes de gerar o DataFrame
data_consolidadas = {
    'COD': df_ano['COD'],
    'Vendedor': df_ano['Vendedor'],
    'Meta_Fat': df_ano['Meta_Fat_q1'].fillna(0) + df_ano['Meta_Fat_q2'].fillna(0),
    'Real_Fat': df_ano['Real_Fat_q1'].fillna(0) + df_ano['Real_Fat_q2'].fillna(0),
    'Meta_Peso': df_ano['Meta_Peso_q1'].fillna(0) + df_ano['Meta_Peso_q2'].fillna(0),
    'Real_Peso': df_ano['Real_Peso_q1'].fillna(0) + df_ano['Real_Peso_q2'].fillna(0),
    'Meta_Pos': df_ano['Meta_Pos_q1'].fillna(0) + df_ano['Meta_Pos_q2'].fillna(0),
    'Real_Pos': df_ano['Real_Pos_q1'].fillna(0) + df_ano['Real_Pos_q2'].fillna(0),
    'Meta_Cad': df_ano['Meta_Cad_q1'].fillna(0) + df_ano['Meta_Cad_q2'].fillna(0),
    'Real_Cad': df_ano['Real_Cad_q1'].fillna(0) + df_ano['Real_Cad_q2'].fillna(0)
}
...
data_consolidadas['Meta_PM'] = (df_ano['Meta_PM_q1'].fillna(0) + df_ano['Meta_PM_q2'].fillna(0)) / 2
data_consolidadas['Real_PM'] = (df_ano['Real_PM_q1'].fillna(0) + df_ano['Real_PM_q2'].fillna(0)) / 2

df = pd.DataFrame(data_consolidadas)

# ✂️ Filtro para deixar apenas o Primeiro Nome de cada vendedor (Formato estável de Julho)
